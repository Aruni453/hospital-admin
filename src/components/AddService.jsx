...
//   to fetch services when in editing state
  ...
  function handleImageChange(e) {
    ...
    // remove existing if user chooses a new file
    setRemoveImage(false);
    setHasExistingImage(false);
  }
//   instruction Helpers
...
// convert selected 12Hrs components to a Data Object
...
// to select user for selecting the past time for that particular date
  function isSelectedDateTimeInPast() {
    const sel = selectedDateTime();
    return sel.getTime() <= Date.now();
  }
//  to add or update the slot
  function addSlot() {
        ...
        <div className={addServiceStyles.toast.container}>
             {toast && (
                 <div className={`${addServiceStyles.toast.toastBase} ${
                        ...
                        toast.type=== "info" ? addServiceStyles.toast.toastInfo : addServiceStyles.toast.toastSuccess
                 } animate-slideIn`}>
          ...
          <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4'>
            <div>
              ...
              <p className={addServiceStyles.header.subtitle}>
                Create a beautiful service card with unique time slot

              </p>
            </div>
            <div className={addServiceStyles.headerActions}>
              <button type='button' onClick={resetForm} disabled={submitting} className={addServiceStyles.buttons.reset}>
                Reset
              </button>
