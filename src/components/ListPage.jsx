...
//   return `${day} ${month} ${y}`;
// }
// // it will normalize date and string
...
// }
// // this function normalize schedule map
...
//   return map;
// }
// // this function will gives past dates first
// // that is nearest date first
...
//     //         {/* Expandable schedule (optional) */}
//     //         {isOpen && sortedDates.length > 0 && (
//     //           <div className={doctorListStyles.scheduleContainer}>
//     //             {sortedDates.map((date) => (
//     //               <div key={date} className={doctorListStyles.scheduleRow}>
//     //                 <span>{formatDateISO(date)}:</span> {scheduleMap[date].join(", ")}
//     //               </div>
//     //             ))}
//     //           </div>
//     //         )}
...
//     <div className={doctorListStyles.doctorInfoContainer}>
//       <div className={doctorListStyles.doctorHeader} onClick={() => toggle(id)}>
//         <div className="flex items-center gap-2 flex-wrap">
//           <h3 className={doctorListStyles.doctorName}>{doc.name}</h3>
//           <span className={doctorListStyles.availabilityBadge(isAvailable)}>
//             <span className={doctorListStyles.availabilityDot(isAvailable)} />
...
import React, { useState, useEffect, useMemo } from 'react';
import { doctorListStyles } from '../assets/dummyStyles';
import { BadgeIndianRupee, EyeClosed, EyeOff, Search, Star, Trash, Users } from 'lucide-react';
...
function ListPage() {
    ...
    try {
      ...
      if (res.ok && body && body.success) {
        const list = Array.isArray(body.data) ? body.data
                     : Array.isArray(body.doctors) ? body.doctors : [];
        ...
        const uniqueDoctors = Array.from(
          new Map(normalized.map(d => [(d._id || d.id), d])).values()
        );
  ...
  const filtered = useMemo(() => {
    ...
    if (filterStatus === "available") {
      list = list.filter(d => (d.availability || "").toLowerCase() === "available");
    } else if (filterStatus === "unavailable") {
      list = list.filter(d => (d.availability || "").toLowerCase() !== "available");
    }
  ...
  return (
    <div className={doctorListStyles.container}>
      <header className={doctorListStyles.headerContainer}>
        <div className={doctorListStyles.headerTopSection}>
          ...
          <div className={doctorListStyles.headerSearchContainer}>
            ...
            <button
              onClick={() => {
                setQuery(""); setExpanded(null); setShowAll(false); setFilterStatus("all");
              }}
              className={doctorListStyles.clearButton}
            >
              Clear
      ...
      <main className={doctorListStyles.gridContainer}>
        {loading && <div className={doctorListStyles.loadingContainer}>Loading Doctors...</div>}
        {!loading && filtered.length === 0 && (
